...
public class Track {
    ...
    private List<double[]> generateTrackCoordinates() {
        ...
        } else if ("S-Shaped".equals(shape)) {
            for (int i = 0; i <= 800; i += 10) {
                double x = i;
                double y = 100 * Math.sin(x / 100.0) + 300;
                coordinates.add(new double[]{x, y});
            }
        } else if ("Complex".equals(shape)) {
            for (int i = 0; i <= 1000; i += 10) {
                double x = i;
                double y = 150 * Math.cos(x / 100.0) + 300;
                coordinates.add(new double[]{x, y});
            }
        }
...
/*
        ...
        for (int i = -1; i <= 1; i++) {
            List<double[]> layer = new ArrayList<>();
            for (double[] coord : baseTrack) {
                double x = coord[0] + i * 15 * Math.cos(Math.atan2(coord[1] - 300, coord[0] - 500));
                double y = coord[1] + i * 15 * Math.sin(Math.atan2(coord[1] - 300, coord[0] - 500));
                layer.add(new double[]{x, y});
            }
            layers.add(layer);
        }
